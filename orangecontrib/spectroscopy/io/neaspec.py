...
class NeaReaderMultiChannelTXT(FileFormat, SpectralFileFormat):
    ...
    def read_spectra(self):
        ...
        with open(path, 'r') as f:
            ...
            table_data_values = np.array([row.strip().split('\t') for row in data[header_length+1:]]) 
        ...
        valid_channel_cols = [i for i, header in enumerate(table_data_headers) if re.match(r'O[1-9][A,P]', header)]
            ...
            for column in range(cols):
                for run in range(runs):
                    ...
                    for channel in range(channels_len):
                        ...
                        channel_data = input_data[input_table_index:input_table_index + depth, valid_channel_cols[channel]]
                        ...
                        out_meta[output_table_index, :] = [data_column, data_row, data_run, channel_name] # np.array([dataColumn, dataRow, dataRun, channelName], dtype='<U10')
        ...
        domain = Orange.data.Domain([], None, metas=metas)
        meta_data = Table.from_numpy(domain, X=np.zeros((len(out_data), 0)), metas=np.asarray(out_meta, dtype=object))
